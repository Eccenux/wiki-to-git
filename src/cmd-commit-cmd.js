...
program
	.usage('-s <site> -r <repo dir> -o <dst file name> [-j <history json>]')
	.description(`Commit JSON to Git.
		
		Creates a file in the Git repository (specified by *repo dir*).
		The output file will have a complete Git history as it existed on Wikipedia
		(the history is committed based on the history in the JSON file).
		...
		wiki2git-commit --site meta.wikimedia.org --repo "./repos/legacy-repo" -o "monobook.js"
		wiki2git-commit --site meta.wikimedia.org --repo "../repos/legacy-repo" -o "monobook.js"

		Required options: ${helper.requiredOptions.join(', ')}.
	`.replaceAll(/\n\t{2}/g, '\n').trim())
	.option('-s, --site <site>', `MediaWiki site domain (e.g. 'en.wikipedia.org').`)
	.option('-r, --repo <repo dir>', 'Path to a new or existing Git repository (name of a subdirectory or a path to a directory).')
	.option('-o, --output <dst file name>', 'Output file name (new JS/CSS file in the Git repo).')
	.option('-j, --json <history json>', `Optional JSON file name for history data (default: 'history.json').`)
	.parse(process.argv);
